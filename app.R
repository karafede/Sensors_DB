                                                       ...
                                                       tabsetPanel(id = "DownloadMode", selected = "ftp",
                                                                   ...
                                                                   tabPanel("csv",
                                                                            ...
                                                                            radioButtons(inputId = 'Ref.Type',
                                                                                         ...
                                                                                         choices = c('Ref', 'Bin.DMPS', 'Bin.APS'),
                                                                                         selected = 'Ref',
                                                                                         inline   = TRUE)),
                                                                   tabPanel("ftp",
                                                                            uiOutput("uiurlref"),
...
server <- function(input, output, session) {
    ...
    observeEvent(input$Selected, {
        ...
        if (input$Selected > 0) {
            ...
            observeEvent({
                ...
                input$FTPMode
                input$RefPollutants
                input$Ref__a_i_p__Pollutants
            },{
                if (!is.null(input$Down.Ref) && input$Down.Ref) {
                        if (input$FTPMode == "ftp") {
                            
                            if (!is.null(input$urlref)) shinyjs::enable("Down_Ref") else shinyjs::disable("Down_Ref")
                            
                        } else {
                            
                            if (input$FTPMode == "csv") {
                                
                                if (!any(is.null(c(input$file1, input$sep, input$quote)))) shinyjs::enable("Down_Ref") else shinyjs::disable("Down_Ref")
                                ...
                                if (input$FTPMode == "SOS") {
                                    ...
                                    if (!is.null(input$Ref__a_i_p__Pollutants)) shinyjs::enable("Down_Ref") else shinyjs::disable("Down_Ref")
                                     ...
                                     tabPanel(
                                         ...
                                         div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                             selectInput(  inputId  = paste0("Sens.raw.unit", i), 
                                                           ...
                                                           choices  = c("V","nA", "ppb", "ug.m-3", "ppm", "mg.m-3"), 
    ...
    REFDATA <- reactive({
        ...
        if (!length(input$coord.ref) > 0 || is.null(input$coord.ref) || input$coord.ref == "" || !grepl(pattern = ",", x = input$coord.ref)) {
        ...
        Plot.Warming   <- reactive({
            ...
            if (length(Index.warm) != 0) {
                ...
                if (input$SavePlot) {
                    ...
                    dev.copy(png,filename = file.path(WDoutput, paste0(AirsensEur.name(), "_", CalSet()$name.sensor, "_Warming.png")), 
                             #units = "cm", 
                             #width = 35.55, 
                             #height = 20,
                             res = 300 
            ...
            if (Outliers.Ref$Forced | Conv$Forced | Cal$Forced) {
                ...
                if (Cal$Forced) { 
                    ...
                    if (!is.null(DF$General)) {
                        ...
                        # initial Calibration with values in input[[paste0("Cal",j)]])) provided that "Method of Prediction" is "Previous calibration"
                        ...
                        if (CalSet()$Cal_Line == "Previous calibration") { #if a for loop is used, replace CalSet()$Cal_Line with input[[paste0("Cal.Line",k)]]
        ...
        plot.drift <- reactive({
            ...
            if (all(is.na(Drift.df()))) {
                ...
                if (input$SavePlot) {
                    ...
                    dev.copy(png, filename = file.path(WDoutput, 
