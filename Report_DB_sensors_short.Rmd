...
author:
- Federico Karagulian, Michel Gerboles, Laurent Spinelle, Maurizio Barbiere, Alex Kotsev, Friedrich Lagler, Annette Borowiak
...
#  scale_color_manual(values=c("blue", "black")) +
  ...
  theme(strip.text.y = element_text(angle = 0)) +
  theme(axis.title.x = element_text(colour="black", size=15),
        axis.text.x  = element_text(angle=0, vjust=0.5, hjust = 0.5, size=15, colour = "red")) +
  theme(axis.title.y = element_text(colour="black", size=15),
        axis.text.y  = element_text(angle=0, vjust=0.5, size=15, colour = "red")) +
...
Caption <- paste0("**Table S3.** Model of OEMs by pollutant, type, openness and price.")
set.caption(Caption)
panderOptions("table.emphasize.rownames", FALSE) # remove row.names from the table
panderOptions("table.split.table", Inf) # to avoid to split tables if rows are too long
panderOptions('table.alignment.default', function(df) ifelse(sapply(df, is.numeric), 'right', 'left')) # right alignment for numeric, left otherwise
...
Caption <- paste0("**Table S4.** Models of Sensor Systems by pollutant, type, openness and price.")
set.caption(Caption)
panderOptions("table.emphasize.rownames", FALSE) # remove row.names from the table
panderOptions("table.split.table", Inf) # to avoid to split tables if rows are too long
panderOptions('table.alignment.default', function(df) ifelse(sapply(df, is.numeric), 'right', 'left')) # right alignment for numeric, left otherwise
...
# 
# models = unique(DB$model[intersect(which(grepl(paste(c("NO2", "O3", "CO", "NO"), collapse = "|"),  x = DB$pollutant)), which(DB$OEM_system == "OEM"))])
# List.Pollutants <- sapply(models, function(i) paste(unique(DB$pollutant[DB$model == i]), collapse = ", "))
# List.Types      <- sapply(models, function(i) paste(unique(DB$type_sensor[DB$model == i]), collapse = ", "))
...
# List_price      <- sapply(models, function(i) if (all(is.na(DB$new_price_EUR[which(DB$model == i)]))) NA else median(DB$price[which(DB$model == i)], na.rm =T) )
# 
# GAS_sensor_model_OEM = data.frame(model = models,
#            List.Pollutants = List.Pollutants,
#            List.Types  = List.Types,
#            List_Refs = List_Refs,
#            List_Open   = List_Open,
...
#            stringsAsFactors = F)
# 
# names(GAS_sensor_model_OEM) <- c("model", "pollutant", "type", "reference", "open/close", "living", "price")
# 
...
# # Model of sensors by for the detection of CO, NO2 and O3. OME/SS indicates Original Equipment Manufacture and Sensor System.
# 
# models = unique(DB$model[intersect(which(grepl(paste(c("NO2", "O3", "CO", "NO"), collapse = "|"),  x = DB$pollutant)), which(DB$OEM_system == "SS"))])
# List.Pollutants <- sapply(models, function(i) paste(unique(DB$pollutant[DB$model == i]), collapse = ", "))
# List.Types      <- sapply(models, function(i) paste(unique(DB$type_sensor[DB$model == i]), collapse = ", "))
# List_Refs       <- sapply(models, function(i) paste(unique(DB$id_ref[DB$model == i]), collapse = ", "))
# List_Open       <- sapply(models, function(i) paste(unique(DB$open_close[DB$model == i]), collapse = ", "))
...
# List_price      <- sapply(models, function(i) if (all(is.na(DB$new_price_EUR[which(DB$model == i)]))) NA else median(DB$price[which(DB$model == i)], na.rm =T) )
# 
# GAS_sensor_model_SS = data.frame(model = models,
#            List.Pollutants = List.Pollutants,
#            List.Types  = List.Types,
#            List_Refs = List_Refs,
#            List_Open   = List_Open,
...
#            stringsAsFactors = F)
# 
# names(GAS_sensor_model_SS) <- c("model", "pollutant", "type", "reference", "open/close", "living", "price")
# 
...
```{r Figure S2, echo = FALSE, warning = FALSE, cache = FALSE, results = 'asis', message = FALSE, comment=FALSE, fig.width = 7, fig.height = 7, fig.cap = "**Figure S2.** Distribution of $R^2$ for OEMs and ensor systems obtained from the calibration against the reference. Records were averaged over a time-scale of 1 minute. Dashed lines indicate the value of 0.7 and 1.0. Numbers in bold indicate the number of open source (blue) and black box (black) records. Names of 'living' and 'non-living' sensors are indicated in black and red color, respectively."}
