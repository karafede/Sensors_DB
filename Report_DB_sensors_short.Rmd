...
# total number of comparison tests reporting the Uncertainty (U) (standard deviation)
...
# total number of comparison tests reporting the Mean Abosolute Error (MAE) 
DB_comparison_MAE <- DB[!(is.na(DB$mae)), ]
n_DB_comparison_MAE <- nrow(DB_comparison_MAE)

DB_comparison_MAE_LAB <- DB_comparison_MAE %>%
  filter(field___lab == "Laboratory Test")
n_DB_comparison_MAE_LAB <- nrow(DB_comparison_MAE_LAB)

DB_comparison_MAE_FIELD <- DB_comparison_MAE %>%
  filter(field___lab == "Feld Test")
n_DB_comparison_MAE_FIELD <- nrow(DB_comparison_MAE_FIELD)


# total number of comparison tests reporting the Mean Normalised root mean squared error (nRMSE) 
DB_comparison_nRMSE <- DB[!(is.na(DB$nrmse__)), ]
n_DB_comparison_nRMSE <- nrow(DB_comparison_nRMSE)

DB_comparison_nRMSE_LAB <- DB_comparison_nRMSE %>%
  filter(field___lab == "Laboratory Test")
n_DB_comparison_nRMSE_LAB <- nrow(DB_comparison_nRMSE_LAB)

DB_comparison_nRMSE_FIELD <- DB_comparison_nRMSE %>%
  filter(field___lab == "Feld Test")
n_DB_comparison_nRMSE_FIELD <- nrow(DB_comparison_nRMSE_FIELD)


# total number of comparison tests reporting the BIAS 
DB_comparison_BIAS <- DB[!(is.na(DB$bias)), ]
n_DB_comparison_BIAS <- nrow(DB_comparison_BIAS)

DB_comparison_BIAS_LAB <- DB_comparison_BIAS %>%
  filter(field___lab == "Laboratory Test")
n_DB_comparison_BIAS_LAB <- nrow(DB_comparison_BIAS_LAB)

DB_comparison_BIAS_FIELD <- DB_comparison_BIAS %>%
  filter(field___lab == "Feld Test")
n_DB_comparison_BIAS_FIELD <- nrow(DB_comparison_BIAS_FIELD)
...
n_FIELD_tests <- c(as.numeric(field_tests),
                   ...
                   as.numeric(n_DB_comparison_MAE_FIELD),
                   as.numeric(n_DB_comparison_nRMSE_FIELD),
                   as.numeric(n_DB_comparison_BIAS_FIELD))
...
n_LAB_tests <- c(as.numeric(lab_tests),
                   ...
                   as.numeric(n_DB_comparison_MAE_LAB),
                   as.numeric(n_DB_comparison_nRMSE_LAB),
                   as.numeric(n_DB_comparison_BIAS_LAB))
...
rownames(df_metrics) <- c("", "$R^{2}$ from calibrations", "$R^{2}$ from comparisons", 
                          ...
                          "RMSE", "Uncertainity (U)", "MAE", "nRMSE", "BIAS")
...
```{r, Figure X, echo = FALSE, warning = FALSE, cache = FALSE, results = 'asis', message = FALSE, comment=FALSE, fig.width = 7, fig.height = 5, fig.cap = "**Figure X.** R2 versus PM reference system"}


# select only Pollutant PM with their reference system measurement
# AAA <- DB[grep(pattern = "PM", x = DB[DB$y %in% c("BAM"), ]$pollutant),    c("pollutant", "r2", "r2_calib", "y")]
r2_PM_ref <- DB[which(DB$y %in% c("BAM", "GRIMM", "DustTrack", "TEOM", "TEOM-FDMS", "TSI OPS", "GRAVIMETRIC", "PARTISOL", "APS") | DB$x %in% c("BAM", "GRIMM", "DustTrack", "TEOM", "TEOM-FDMS", "TSI OPS", "GRAVIMETRIC","PARTISOL", "APS")), 
                c("pollutant", "r2", "r2_calib", "x", "y")]


r2_PM_ref$Refnames <- sapply(1:nrow(r2_PM_ref), function(i) ifelse(r2_PM_ref[i,"x"] == "Sensor",
                            as.character(r2_PM_ref[i,"y"]), as.character(r2_PM_ref[i,"x"])))

r2_PM_ref[which(r2_PM_ref$Refnames == "TEOM-FDMS")  , "Refnames"] <- "TEOM"
# quantile(r2_PM_ref[which(r2_PM_ref$Refnames == "TEOM")  , "r2"], probs = c(0.25,0.5,0.75), na.rm = T)
# quantile(r2_PM_ref[which(r2_PM_ref$Refnames == "BAM")  , "r2"], probs = c(0.25,0.5,0.75), na.rm = T)
# quantile(r2_PM_ref[which(r2_PM_ref$Refnames == "GRIMM")  , "r2"], probs = c(0.25,0.5,0.75), na.rm = T)
### COMPARISON #####
       
# compute frequency of records for each model of sensor
r2_PM_ref_COMPARISON <- r2_PM_ref[!is.na(r2_PM_ref$r2)  ,c("r2","Refnames") ]
counts_r2_PM_ref <- r2_PM_ref_COMPARISON %>%
  dplyr::group_by(Refnames) %>%
  summarise(counts = length(Refnames))

q <- ggplot(data = r2_PM_ref_COMPARISON, 
            aes(Refnames, r2, fill = Refnames)) +
  theme_bw() +
  geom_boxplot(aes(fill = Refnames), position = position_dodge2(preserve = "single")) +
  geom_text(data = counts_r2_PM_ref, aes(x = Refnames, y = 0.85, label = counts, fontface="bold"), size = 5, show.legend = FALSE) +
  theme(legend.text = element_text(colour="black", size = 10, face = "bold")) +
  theme(axis.text.x=element_text(angle=45,hjust=1,vjust=1, size=10)) +
  theme(axis.text.x=element_text(size=13,face="bold", colour = "black")) +
  theme(axis.title.x = element_blank()) +                                     
  ...
  theme(axis.title.y = element_text(face="bold", colour="black", size=14),
        axis.text.y  = element_text(angle=0, vjust=0.5, size=14, colour="black")) +
  ggtitle(expression(paste(R^2,  " for PM sensors by reference system (COMPARISON)"))) + 
  theme(plot.title = element_text(lineheight=.8, face="bold", size = 13, hjust=0.5)) 
q

### CALIBRATION ####

r2_PM_ref_CALIBRATION <- r2_PM_ref[!is.na(r2_PM_ref$r2_calib)  ,c("r2_calib","Refnames") ]
counts_r2_PM_ref <- r2_PM_ref_CALIBRATION %>%
  dplyr::group_by(Refnames) %>%
  summarise(counts = length(Refnames))

q <- ggplot(data = r2_PM_ref_CALIBRATION, 
            aes(Refnames, r2_calib, fill = Refnames)) +
  theme_bw() +
  geom_boxplot(aes(fill = Refnames), position = position_dodge2(preserve = "single")) +
  geom_text(data = counts_r2_PM_ref, aes(x = Refnames, y = 0.85, label = counts, fontface="bold"), size = 5, show.legend = FALSE) +
  theme(legend.text = element_text(colour="black", size = 10, face = "bold")) +
  ...
  theme(axis.title.y = element_text(face="bold", colour="black", size=14),
        axis.text.y  = element_text(angle=0, vjust=0.5, size=14, colour="black")) +
  ggtitle(expression(paste(R^2, " for PM sensors by reference system (CALIBRATION)"))) + 
  theme(plot.title = element_text(lineheight=.8, face="bold", size = 13, hjust=0.5)) 
q



#======== select all R2 from comparison with referecne instruments (only FIELD tests)

DB_r2_max_conc <- DB[!(is.na(DB$r2 & DB$max_concentration)), c("r2", "max_concentration", "pollutant", "season", "field___lab")]
DB_r2_max_conc <- DB_r2_max_conc %>%
  dplyr::filter(field___lab == "FIELD") 
  #dplyr::filter(max_concentration < 100) 
...
plot <- ggplot(DB_r2_max_conc, aes(y = r2, x = max_concentration)) +
  theme_bw() +
  geom_point(size = 2) +
  geom_smooth(data = DB_r2_max_conc, method="lm", formula = my.formula) +  # Add linear regression line
  # facet_grid(season ~ .) + 
  xlab("max PM reference concentration") +
  ylab(expression(paste(R^2),size=40)) + 
  theme(strip.text = element_text(size = 15, face="bold")) +
  theme(strip.text.y = element_text(angle = 0)) +
  theme(axis.title.x = element_text(colour="black", size=15),
  ...
  theme(axis.title.y = element_text(colour="black", size=15),
        axis.text.y  = element_text(angle=0, vjust=0.5, size=15, colour = "black")) +
  ggtitle(expression(paste("Relation between ",  R^2 , " and max concentratio of PM from reference instrument"))) +
  theme(plot.title = element_text(lineheight=.8, face="bold", size = 10, hjust = 0.5)) 
  plot
